...
parser.add_argument('--num_local_experts', type=float, default=2)
...
import os
#print("SD: world_size=", os.environ['OMPI_COMM_WORLD_SIZE'])
parallel_env = system.init_data_model_parallel(backend='nccl' if args.device == 'cuda' else 'gloo')
print("parallel_env =", parallel_env.global_size)
...
class ComplexModel(torch.nn.Module):
    def __init__(self, num_layers=1):
        ...
        for i in range(self._num_layers):
            self._moe_layers.append(tutel_moe.moe_layer(
                gate_type = {'type': 'top', 'k': top_value, 'fp32_gate': args.fp32_gate, 'capacity_factor': args.capacity_factor},
                experts = {'type': 'ffn', 'count_per_node': num_local_experts, 'hidden_size_per_expert': hidden_size, 'activation_fn': lambda x: F.relu(x)},
                model_dim = model_dim,
                scan_expert_func = lambda name, param: setattr(param, 'skip_allreduce', True),
        ...
        self._moe_layers = torch.nn.Sequential(*self._moe_layers)
        # Summary of different parameter types: gate, local_experts
        '''
        local_count = self._num_layers * sum([torch.numel(param) for name, param in self._moe_layers.get_parameter_iterator(param_type='local_experts')])
        shared_count = self._num_layers * sum([torch.numel(param) for name, param in self._moe_layers.get_parameter_iterator(param_type='gate')])
        dist_print('[Statistics] param count for MoE local_experts = %s, param count for MoE gate = %s.\n' % (local_count, shared_count))
        '''
    def forward(self, input):
        if args.megablocks_size > 0:
          result = input
          for i in range(self._num_layers):
            result = self._moe_layers[i](result, megablocks_size=args.megablocks_size)
        else:
          result = input
          for i in range(self._num_layers):
            result = self._moe_layers[i](result)
        result = F.log_softmax(torch.sum(result, dim=2), dim=1)
        return result


model = ComplexModel(args.num_layers).to(device)
dist_print(model)
...
for i in range(num_steps):
    ...
    print("SD: GBE --", num_global_experts)
    mm_ceof, cap_ceof = 1 if args.eval else 3, min(args.top, num_global_experts)
    print("SD: model and training parameters--")
    print(f"batch_size={batch_size}, num_tokens={num_tokens}, model_dim={model_dim}, hidden_size={hidden_size}, mm_ceof={mm_ceof}, cap_ceof={cap_ceof}")
    tflops = args.num_layers * (batch_size * num_tokens * model_dim * hidden_size) * 4 * mm_ceof * cap_ceof * 1e-12 / (t_stop - t_start)
    dist_print('STEP-%s: loss = %.5f, step_time = %.6f sec, perf = %.2f tflops.' % (i, float(loss.data), t_stop - t_start, tflops))
    #print('STEP-%s: loss = %.5f, step_time = %.6f sec, perf = %.2f tflops.' % (i, float(loss.data), t_stop - t_start, tflops))
    if i + 10 >= num_steps:
